        ...
        return (
          <div key={player.id} className="flex flex-col items-center group w-24 sm:w-28 relative">
            ...
            <div className="relative mb-2 flex flex-col items-center transition-all duration-300 group-hover:-translate-y-3 z-20">
              ...
              <div className={`rounded-full border-4 shadow-xl overflow-hidden flex items-center justify-center bg-white relative ${
                rank === 1 ? 'border-amber-300 w-16 h-16 sm:w-20 sm:h-20' : 
                rank === 2 ? 'border-slate-300 w-14 h-14 sm:w-16 sm:h-16' : 
                'border-orange-300 w-14 h-14 sm:w-16 sm:h-16'
              }`}>
                {player.photoURL ? (
                  <img src={player.photoURL} alt="" className="w-full h-full object-cover" />
                ) : (
                  <span className="font-black text-2xl text-slate-300">{player.displayName?.charAt(0) || '?'}</span>
                )}
              </div>
              {isCurrentUser && (
            ...
            {/* Podium Block */}
            <div className={`w-full ${heights[index]} bg-gradient-to-b ${colors[index]} rounded-t-xl border-t border-l border-r flex flex-col items-center justify-start pt-3 shadow-xl relative overflow-hidden backdrop-blur-sm transition-all duration-300 group-hover:brightness-110`}>
              <div className="absolute inset-0 bg-white/20"></div>
              {/* Glass reflection */}
              <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-white/30 to-transparent"></div>
              
              <span className="font-black text-3xl sm:text-4xl opacity-90 drop-shadow-sm relative z-10">{rank}</span>
              ...
              <div className="w-full px-1 text-center mt-auto mb-3 relative z-10">
                ...
                <div className="text-xs sm:text-sm font-black mt-0.5">
                  {getScore(player).toLocaleString()} <span className="text-[9px] opacity-75 font-bold">{getScoreLabel()}</span>
                </div>
              </div>
            </div>
          </div>
