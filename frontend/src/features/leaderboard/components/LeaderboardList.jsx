...
export default function LeaderboardList({ leaders, loading, user, activeTab }) {
  ...
  return (
    <div className="flex flex-col">
      {top3.length > 0 && <LeaderboardPodium top3={top3} user={user} activeTab={activeTab} />}
      ...
      <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden mt-2">
        <div className="flex justify-between items-center border-b border-slate-100/80 px-4 py-3 bg-slate-50/50">
          <span className="text-[10px] sm:text-xs font-black text-slate-400 w-12 text-center uppercase tracking-wider">
            Rank
          </span>
          <span className="text-[10px] sm:text-xs font-black text-slate-400 flex-1 px-2 uppercase tracking-wider">
            Manager
          </span>
          <span className="text-[10px] sm:text-xs font-black text-slate-400 text-right uppercase tracking-wider">
            {activeTab === 'club' ? 'EXP' : 'Points'}
          </span>
        </div>
