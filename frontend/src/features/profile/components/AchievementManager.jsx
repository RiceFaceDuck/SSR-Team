...
const getRarityStyles = (rarity, unlocked) => {
  ...
  switch (rarity) {
    case 'legendary':
      return 'bg-gradient-to-r from-amber-50 to-orange-50 border-amber-300 shadow-amber-200/50 shadow-md';
    case 'epic':
      return 'bg-gradient-to-r from-purple-50 to-fuchsia-50 border-purple-300 shadow-purple-200/50 shadow-sm';
    case 'rare':
...
const getIconColor = (rarity, unlocked) => {
  ...
  switch (rarity) {
    ...
    case 'epic':
      return 'bg-gradient-to-br from-purple-500 to-fuchsia-500 text-white shadow-sm';
    case 'rare':
      return 'bg-gradient-to-br from-blue-400 to-cyan-500 text-white shadow-sm';
    default:
          ...
          <div className="mb-4 p-4 bg-gradient-to-br from-indigo-900 to-blue-900 border border-indigo-500/30 rounded-xl flex justify-between items-center shadow-lg shadow-indigo-900/20">
            <div>
              ...
              <div className="font-black text-white text-lg tracking-wide">
                {userData?.equippedTitle || 'ไม่มีฉายา'}
              </div>
            </div>
            ...
            {loading ? (
              ...
              achievements.map((achv) => {
                ...
                return (
                  <div
                    key={achv.id}
                    className={`flex items-center justify-between p-3 rounded-2xl border transition-all ${getRarityStyles(achv.rarity, achv.unlocked)}`}
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`p-2.5 rounded-xl ${getIconColor(achv.rarity, achv.unlocked)}`}
                      >
                        {getIconComponent(achv.iconType)}
                      </div>
                      <div>
                        <div
                          className={`font-black text-sm tracking-wide ${achv.unlocked ? 'text-slate-800' : 'text-slate-500'}`}
                        >
                          {achv.title}
                        </div>
                    ...
                    {achv.unlocked ? (
                      ...
                      <div className="text-[10px] font-bold text-slate-400 px-2 uppercase tracking-widest">
                        ล็อค
                      </div>
                    )}
                  </div>
