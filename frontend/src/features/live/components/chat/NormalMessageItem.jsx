...
export default function NormalMessageItem({ msg, isMe }) {
  ...
  if (msg.clubTier === 4) {
    frameClass = 'bg-gradient-to-br from-amber-400 to-orange-600 border-2 border-amber-300 shadow-[0_0_10px_rgba(251,191,36,0.5)]';
    nameClass = 'text-orange-500 font-black drop-shadow-sm';
  } else if (msg.clubTier === 3) {
    frameClass = 'bg-gradient-to-br from-fuchsia-500 to-purple-600 border-2 border-fuchsia-300 shadow-[0_0_8px_rgba(192,38,211,0.4)]';
    nameClass = 'text-purple-500 font-bold';
  } else if (msg.clubTier === 2) {
    frameClass = 'bg-gradient-to-br from-blue-500 to-indigo-600 border-2 border-blue-300 shadow-[0_0_5px_rgba(59,130,246,0.3)]';
  ...
  if (isSuper) {
    frameClass = 'bg-gradient-to-br from-amber-200 to-yellow-300 border-2 border-yellow-400 shadow-sm';
    nameClass = 'text-amber-500 font-black';
  }

  return (
    <div className={`flex w-full gap-2 ${isMe ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300`}>
      ...
      <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[75%]`}>
        <div className={`flex items-center gap-1.5 px-1 mb-0.5 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
          <span className={`text-[10px] ${nameClass}`}>
            {msg.userName}
          </span>
          {msg.equippedTitle && (
        ...
        <div className={`px-3 py-2 text-sm break-words w-fit ${
          isSuper 
