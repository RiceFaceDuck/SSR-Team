...
export default function ChatMessageList({ messages }) {
  ...
  const allSuperChats = messages
    .filter((msg) => msg.isSuperChat && msg.pinnedUntil && getTimeMs(msg.pinnedUntil) > now)
    .sort((a, b) => getTimeMs(a.startTime) - getTimeMs(b.startTime));

  // แยกตัวที่กำลังแสดงผล (startTime <= now) และตัวที่กำลังรอคิว (startTime > now)
  const activeSuperChats = allSuperChats.filter(
    (msg) => !msg.startTime || getTimeMs(msg.startTime) <= now
  );
  const waitingSuperChats = allSuperChats.filter(
    (msg) => msg.startTime && getTimeMs(msg.startTime) > now
  );
  ...
  return (
    <div className="flex-1 overflow-y-auto custom-scrollbar p-2 flex flex-col gap-3 relative">
        ...
        <div className="sticky top-0 z-20 flex gap-2 mb-2 overflow-x-auto pb-2 custom-scrollbar">
          ...
          {activeSuperChats.map((msg) => {
            ...
            return (
              <div
                key={`pinned-${msg.id}`}
                className="shrink-0 w-[240px] bg-gradient-to-r from-amber-200 via-yellow-300 to-amber-200 border-2 border-yellow-400 p-2 rounded-xl shadow-lg animate-pulse-slow"
              >
                <div className="flex justify-between items-center mb-1">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-[10px] font-black text-amber-900 uppercase truncate max-w-[120px]">
                      {msg.userName}
                    </span>
                  </div>
                  <span className="text-[9px] font-bold text-amber-700 bg-white/50 px-1.5 py-0.5 rounded-full">
                    {timeLeft}s
                  </span>
                </div>
            ...
            <div className="shrink-0 flex items-center pr-2 border-l border-slate-200 ml-1 pl-2 gap-2">
              ...
              {waitingSuperChats.map((msg, idx) => {
                const duration = Math.floor(
                  (getTimeMs(msg.pinnedUntil) - getTimeMs(msg.startTime)) / 1000
                );
                return (
                  <div
                    key={`waiting-${msg.id}`}
                    className="shrink-0 w-[180px] bg-slate-50 border-2 border-slate-200/60 p-2 rounded-xl shadow-sm opacity-90 flex flex-col justify-center"
                  >
                    <div className="flex justify-between items-center mb-1">
                      <div className="flex items-center gap-2">
                        <img
      ...
      {messages.length === 0 ? (
        ...
        messages
          .filter((msg) => !msg.isSuperChat || !msg.startTime || getTimeMs(msg.startTime) <= now)
          .map((msg) => {
            ...
            if (isSystem) {
              return (
                <div
                  key={msg.id}
                  className="text-center text-xs font-medium text-amber-600 bg-amber-50 py-1 px-3 rounded-full mx-auto my-1"
                >
                  {msg.text}
                </div>
              );
