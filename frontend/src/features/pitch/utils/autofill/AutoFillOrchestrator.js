...
export const runAutoFillEngine = ({
  marketPlayers = [],
  mySquad = [],
  formation,
  budgetLeft,
  ...
  positionsToFill.forEach((pos) => {
    while (currentCount[pos] < totalRequired[pos]) {
      ...
      const slotsLeft = Object.keys(totalRequired).reduce(
        (sum, p) => sum + (totalRequired[p] - currentCount[p]),
        0
      );
      const budgetPerSlot =
        slotsLeft > 0 ? (currentBaseBudget / slotsLeft) * 1.5 : currentBaseBudget; // อนุญาตให้เกินค่าเฉลี่ยได้ 50%
      ...
      if (!bestPlayer) {
        // หาตัวถูกที่สุด
        const validFallbacks = marketPlayers
          .filter((p) => {
        ...
        if (fallbackPlayer) {
          ...
          currentCount[pos]++;
          teamCounts[fallbackPlayer.team || 'UNK'] =
            (teamCounts[fallbackPlayer.team || 'UNK'] || 0) + 1;
          isModified = true;
        } else {
  ...
  if (isModified || newSquad.some((p) => p.isStarting)) {
    ...
    return {
      ...
      message: isComplete
        ? 'AI วิเคราะห์ฟอร์มและจัดทีมแบบกระจายงบประมาณเสร็จสิ้น!'
        : `จัดทีมได้ ${newSquad.length}/15 คน (นักเตะในตลาดหรือเงินไม่พอ)`,
    };
  } else {
    return {
      success: false,
