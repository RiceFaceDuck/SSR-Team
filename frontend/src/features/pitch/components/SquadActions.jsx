...
const SquadActions = ({
  ...
  const { cooldowns, executeActionWithCooldown, isNoAdsMode } = useActionCooldown({
    autoPick: 0,
    reset: 0,
    saveTeam: 0,
  });
  ...
  const renderButtonContent = (
    ...
    if (isLoading) {
      return (
        <>
          <svg
            className="animate-spin w-4 h-4 text-[#0a192f] shrink-0"
          ...
          <span className="opacity-80 flex items-baseline gap-1">
            ...
            {managerBonus > 0 && (
              <span
                ...
                title={`Manager Bonus: +${managerBonus}m`}
              >
                (+{managerBonus}m)
              </span>
            )}
