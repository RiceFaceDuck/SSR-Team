    ...
    <div className="space-y-2">
      {rankedMembers.map((m) => {
        ...
        return (
          <div
            key={m.id}
            className={`flex items-center p-3 rounded-xl border transition-all hover:scale-[1.01] ${isMe ? 'border-indigo-300 bg-indigo-50 shadow-sm' : 'border-slate-100 bg-white hover:border-slate-200'}`}
          >
            <div className="w-10 flex flex-col items-center justify-center">
              <span
                className={`font-black text-lg leading-none ${m.rank <= 3 ? 'text-amber-500' : 'text-slate-400'}`}
              >
                {m.rank}
              </span>
            ...
            <div className="flex-1 px-3 flex items-center gap-3 overflow-hidden">
              ...
              <div className="flex flex-col overflow-hidden">
                <span
                  className={`font-bold text-sm truncate ${isMe ? 'text-indigo-900' : 'text-slate-800'}`}
                >
              ...
              {isMe && (
                <span className="bg-indigo-600 text-white text-[9px] px-1.5 py-0.5 rounded-md font-black uppercase ml-auto">
                  You
                </span>
              )}
            </div>
            ...
            <div className="text-right flex flex-col items-end pl-2 border-l border-slate-100/50">
              <span
                className={`font-black text-lg leading-none ${isMe ? 'text-indigo-600' : 'text-slate-700'}`}
              >
                {m.userPoints || 0}
              </span>
