...
export default function FriendManager() {
  const { userData, friends, isFriendsLoading, loadFriends, searchFriend, addFriend, acceptFriend, removeOrRejectFriend } = useUserStore();
      ...
      {searchResult && (
        <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 mb-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <img src={searchResult.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${searchResult.id}`} alt="avatar" className="w-10 h-10 rounded-full border border-slate-300" />
            <div>
              <div className="font-bold text-sm text-slate-800">{searchResult.displayName || 'ไม่มีชื่อ'}</div>
              <div className="text-[10px] text-slate-500">UID: {searchResult.id}</div>
            </div>
          </div>
          {friends.find(f => f.uid === searchResult.id) ? (
            <div className="text-xs font-bold text-slate-400 bg-slate-200 px-2 py-1 rounded-md">ส่งคำขอไปแล้ว/เป็นเพื่อนกันแล้ว</div>
          ) : (
            <button 
      ...
      {isFriendsLoading ? (
        ...
        <div className="space-y-4">
          ...
          {pendingRequests.length > 0 && (
            <div>
              <h4 className="text-xs font-bold text-slate-500 uppercase mb-2 border-b border-slate-100 pb-1">คำขอเป็นเพื่อน ({pendingRequests.length})</h4>
              <div className="space-y-2">
                {pendingRequests.map(req => (
                  <div key={req.uid} className="flex justify-between items-center bg-indigo-50/50 p-2 rounded-lg border border-indigo-100">
                    <div className="flex items-center gap-2">
                      <img src={req.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${req.uid}`} alt="avatar" className="w-8 h-8 rounded-full border border-indigo-200" />
                      <span className="font-bold text-sm text-slate-800">{req.displayName}</span>
                    </div>
                    <div className="flex gap-1">
                      <button onClick={() => handleAccept(req.uid)} className="bg-emerald-500 text-white p-1.5 rounded-md hover:bg-emerald-600">
                        <UserCheck size={14} />
                      </button>
                      <button onClick={() => handleRemove(req.uid, true)} className="bg-red-500 text-white p-1.5 rounded-md hover:bg-red-600">
                        <UserX size={14} />
                      </button>
                    </div>
                  </div>
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-500 uppercase mb-2 border-b border-slate-100 pb-1 flex justify-between">
              ...
              {requestedFriends.length > 0 && <span className="text-indigo-400 font-normal">รอตอบรับ {requestedFriends.length}</span>}
            </h4>
            
            {acceptedFriends.length === 0 ? (
              <div className="text-center text-xs text-slate-400 py-2 italic">ยังไม่มีเพื่อน ลองค้นหาและเพิ่มเพื่อนดูสิ!</div>
            ) : (
              <div className="space-y-2">
                {acceptedFriends.map(friend => (
                  <div key={friend.uid} className="flex justify-between items-center bg-white p-2 rounded-lg border border-slate-100 hover:border-slate-200 transition-colors">
                    <div className="flex items-center gap-2">
                      <img src={friend.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${friend.uid}`} alt="avatar" className="w-8 h-8 rounded-full border border-slate-200" />
                      <span className="font-bold text-sm text-slate-700">{friend.displayName}</span>
                    </div>
                    <button onClick={() => handleRemove(friend.uid, false)} className="text-slate-400 hover:text-red-500 p-1 transition-colors" title="ลบเพื่อน">
                      <UserMinus size={14} />
                    </button>
                  </div>
                ))}
