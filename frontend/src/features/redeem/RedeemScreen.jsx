...
export default function RedeemScreen() {
  ...
  return (
    <div className="p-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24 max-w-4xl mx-auto min-h-screen">
      ...
      {}
      {/* Header Section */}
      <div className="flex justify-between items-start mb-8 bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
        ...
        <div className="flex flex-col items-end gap-2.5">
          ...
          <div className="bg-gradient-to-r from-slate-900 to-slate-800 text-white px-4 py-2 rounded-2xl shadow-lg flex items-center gap-2 border border-slate-700 hover:scale-105 transition-transform cursor-pointer">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">ยอดเงิน</span>
            <div className="flex items-center gap-1">
              <span className="font-black text-lg text-amber-400 leading-none">{balls?.toLocaleString() || 0}</span>
          ...
          <button 
            ...
            className="text-[10px] font-bold text-slate-500 hover:text-indigo-600 bg-slate-50 hover:bg-indigo-50 px-3 py-1.5 rounded-full border border-slate-200 hover:border-indigo-200 transition-all active:scale-95 flex items-center gap-1"
          >
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-3xl p-12 text-center shadow-sm border border-slate-100 flex flex-col items-center gap-4">
          <PackageOpen size={48} className="text-slate-300" />
          <div>
            <h3 className="text-lg font-bold text-slate-700">ยังไม่มีสินค้าในร้านค้า</h3>
        ...
        <div className="space-y-10">
          ...
          {gachaRewards.length > 0 && (
            <section className="relative">
              <div className="absolute inset-0 bg-gradient-to-br from-purple-600/5 to-indigo-600/5 rounded-[2.5rem] -m-6 p-6 -z-10 border border-purple-500/10"></div>
              
              <div className="flex items-center gap-2 mb-4 px-2">
                <Sparkles className="text-purple-500" size={20} />
                <h3 className="text-lg font-black text-slate-800 tracking-tight">โซนเสี่ยงดวง (Mystery Box)</h3>
                <span className="bg-purple-100 text-purple-700 text-[10px] font-bold px-2 py-0.5 rounded-full ml-2 uppercase tracking-wider">Hot</span>
              </div>
      ...
      {wonItemModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden transform transition-all scale-100 animate-in zoom-in-95 duration-300 flex flex-col relative border-4 border-amber-400">
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-6 bg-amber-400 rounded-b-xl flex justify-center items-end pb-1 shadow-inner">
              <span className="text-[10px] font-black text-amber-900 uppercase tracking-widest">Jackpot</span>
            </div>
            ...
            <div className="p-8 text-center pt-10">
              ...
              <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-6 border border-amber-200 shadow-inner mb-6 transform hover:scale-105 transition-transform">
                ...
                {wonItemModal.rarity && (
                  <span className="inline-block mt-2 bg-amber-500 text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-sm">
              ...
              <button
                ...
                className="w-full px-4 py-3.5 text-sm font-bold text-white bg-slate-900 hover:bg-slate-800 rounded-xl transition-all shadow-md active:scale-95"
              >
