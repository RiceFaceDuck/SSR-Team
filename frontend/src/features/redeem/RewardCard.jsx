...
export function RewardCard({ reward, title, cost, imageSlot, onSuccess }) {
  ...
  return (
    <div
      className={`bg-white rounded-2xl p-4 flex flex-col h-full relative overflow-hidden group transition-all duration-300 border shadow-[0_8px_30px_rgb(0,0,0,0.04)] 
    ...
    >
      ...
      <div className="absolute top-6 left-6 right-6 flex justify-between items-start z-10 pointer-events-none">
        ...
        {displayStock !== undefined && (
          <span
            className={`text-[10px] font-black px-2 py-1 rounded-full shadow-sm backdrop-blur border ${
              isOutOfStock
                ? 'bg-red-50/90 text-red-500 border-red-200'
                : displayStock <= 5
                  ? 'bg-orange-50/90 text-orange-500 border-orange-200'
      ...
      <div
        className={`w-full aspect-video rounded-xl mb-4 flex items-center justify-center text-5xl bg-slate-50 transition-all duration-500 group-hover:scale-[1.02] overflow-hidden relative border border-slate-200 ${imageOpacity}`}
      >
        {isGacha && (
        ...
        {displayImage ? (
          ...
          imageSlot ||
          (isGacha ? (
            <Package className="text-purple-300 w-16 h-16" />
          ) : (
      ...
      <div className="flex-1 flex flex-col">
        <h3
          className={`font-bold text-sm leading-tight line-clamp-2 ${isOutOfStock ? 'text-slate-400' : 'text-slate-800'}`}
        >
          {displayTitle}
        </h3>
        {displayDesc && (
      ...
      <div
        className={`mt-4 flex items-center justify-between gap-3 pt-4 border-t ${isOutOfStock ? 'border-slate-100' : 'border-slate-100 group-hover:border-indigo-200 transition-colors'}`}
      >
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-1">
            <span
              className={`font-black text-base tracking-tight ${isNotEnoughBalls ? 'text-rose-500' : 'text-amber-500'}`}
            >
