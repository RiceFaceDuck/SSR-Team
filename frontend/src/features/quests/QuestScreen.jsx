...
export default function QuestScreen() {
  ...
  const { quests, isLoading, fetchActiveQuests, claimReward } = useQuestStore();
  ...
  const handleClaim = async (quest) => {
    if (!userData?.uid) {
      showToast('กรุณาเข้าสู่ระบบก่อนรับรางวัล', 'error');
      return;
    }
    ...
    if (res.success) {
      ...
      showToast(`รับ ${quest.rewardBalls} ⚽ สำเร็จ!`, 'success');
    } else {
      showToast(res.message || 'เกิดข้อผิดพลาดในการรับรางวัล', 'error');
    }
