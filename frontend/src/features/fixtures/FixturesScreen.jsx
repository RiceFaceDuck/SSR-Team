  ...
  const groupedFixtures = useMemo(() => {
    ...
    return fixtures.reduce((acc, match) => {
      ...
      const findTeam = (name) => {
        ...
        const partialMatch = teams.find(t => name.toLowerCase().includes(t.name.toLowerCase()) || t.name.toLowerCase().includes(name.toLowerCase()));
        ...
        <div className="space-y-2 pb-6 min-h-[50vh]">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 text-indigo-500">
              ...
              <p className="font-bold text-sm text-slate-500 animate-pulse">กำลังโหลดตารางแข่งขัน...</p>
            </div>
          ) : error ? (
