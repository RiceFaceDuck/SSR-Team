...
const MarketFilters = ({
  ...
  const hasFilters =
    searchQuery !== '' || sortBy !== 'price-desc' || activeTab !== 'ALL' || selectedClub !== 'ALL';
      ...
      <div className="flex gap-2 relative z-10">
        ...
        <div className="relative shrink-0 w-[100px]">
          ...
          <select
            ...
            className="w-full bg-[#0d2341] border border-[#1a365d] shadow-inner text-white rounded-lg pl-8 pr-2 py-2.5 text-[11px] font-bold outline-none focus:ring-2 focus:ring-[#3b82f6] focus:border-[#3b82f6] appearance-none cursor-pointer transition-all"
          >
            <option value="ALL">ทุกสโมสร</option>
            {availableClubs?.map((club) => (
              <option key={club} value={club}>
                {formatTeamShortName(club)}
              </option>
            ))}
          </select>
        </div>
