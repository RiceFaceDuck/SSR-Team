...
import ArsBadge from './badges/ArsBadge';
...
export const getTeamInfo = (name, shortName) => {
  ...
  if (shortName) {
    const byCode = Object.values(TEAM_COLORS_BY_NAME).find(t => t.code === shortName?.toUpperCase());
...
export default function TeamBadge({ teamName, shortName, size = 'md', className = '', showText = true }) {
  ...
  if (teamInfo.code === 'ARS') {
    return <ArsBadge size={size} className={className} showText={showText} />;
  }
