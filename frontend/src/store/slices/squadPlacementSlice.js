...
export const squadPlacementSlice = (set, get) => ({
  ...
  startPlacement: (player) => {
    ...
    let currentSquadObjects = currentSquad
    ...
    if (
      pendingTargetSlot &&
      String(pendingTargetSlot) !== 'bench' &&
      String(pendingTargetSlot) !== 'null'
    ) {
      const playerToReplaceSquadItem = currentSquad.find(
        (sq) => String(sq.slotIndex) === String(pendingTargetSlot) && sq.isStarting
      );
      if (playerToReplaceSquadItem) {
        const playerToReplace = currentSquadObjects.find(
          (p) => String(p.sku) === String(playerToReplaceSquadItem.playerId)
        );
        if (playerToReplace) {
          currentSquadObjects = currentSquadObjects.filter(
            (p) => String(p.sku) !== String(playerToReplace.sku)
          );

          let priceToRefund = parseFloat(playerToReplaceSquadItem.purchasePrice);
          if (isNaN(priceToRefund)) priceToRefund = parseFloat(playerToReplace.price) || 0;
          if (playerToReplaceSquadItem.appliedCardId) {
            const card = get().availableCards?.find(
              (c) => c.id === playerToReplaceSquadItem.appliedCardId
            );
    ...
    if (pendingTargetSlot) {
      ...
      return {
        ...
        message: 'ซื้อและลงสนามในตำแหน่งที่เลือกเรียบร้อย!',
      };
    }
  ...
  confirmPlacement: (slotIndex) => {
    ...
    const actualSlotIndex =
      slotIndex === 'bench' || slotIndex === 'null' || slotIndex === null ? null : slotIndex;

    if (actualSlotIndex !== undefined && actualSlotIndex !== null) {
      const existingStarterIndex = newSquad.findIndex(
        (p) => String(p.slotIndex) === String(actualSlotIndex) && p.isStarting
      );
      if (existingStarterIndex !== -1) {
        ...
        const oldPlayer = players.find(
          (p) => String(p.sku) === String(oldPlayerSquadData.playerId)
        );

        if (oldPlayer) {
          let oldPlayerPrice = parseFloat(oldPlayerSquadData.purchasePrice);
          if (isNaN(oldPlayerPrice)) oldPlayerPrice = parseFloat(oldPlayer.price) || 0;

          if (oldPlayerSquadData.appliedCardId) {
            const card = get().availableCards?.find(
              (c) => c.id === oldPlayerSquadData.appliedCardId
            );
            if (card && card.effectLogic?.type === 'PRICE_REDUCTION') {
