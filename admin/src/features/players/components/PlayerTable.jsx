    ...
    () => [
      {
        ...
        cell: (row) => (
          <div className="flex flex-col gap-1">
            ...
            {row.dataSource === 'API' ? (
              <span className="bg-emerald-100 text-emerald-700 text-[10px] px-1.5 py-0.5 rounded border border-emerald-200 font-bold w-max">
                API
              </span>
            ) : row.dataSource ? (
              <span className="bg-amber-100 text-amber-700 text-[10px] px-1.5 py-0.5 rounded border border-amber-200 font-bold w-max">
                {row.dataSource}
              </span>
            ) : (
      ...
      { header: 'สโมสร', accessorKey: 'team' },
      {
        ...
        className: 'text-green-600 font-medium w-20',
        cell: (row) =>
          row.displayPrice ? String(row.displayPrice).replace('£', '') : `${row.price || 0}m`,
      },
      { header: 'สถานะ', cell: (row) => <StatusBadge status={row.status} /> },
      {
        ...
        cell: (row) => (
          ...
          >
            ...
            >
              <CloudDownload
                className={`w-4 h-4 ${isCheckingRow === row.id ? 'animate-bounce' : ''}`}
              />
            </button>
  ...
  return (
    <>
      <div className="text-sm text-gray-500 px-1 flex justify-between items-center mb-2">
        <div>
          แสดง <span className="font-bold text-gray-900">{players.length}</span> รายการ
        </div>
        {selectedTeam !== 'All' && (
          <div className="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-md text-xs font-bold border border-indigo-100">
            {selectedTeam}
          </div>
        )}
      </div>
        ...
        <DataTable
          ...
          emptyMessage={
            searchTerm || selectedTeam !== 'All'
              ? `ไม่พบนักเตะในเงื่อนไขการค้นหานี้`
              : 'ยังไม่มีข้อมูลนักเตะในระบบ'
          }
