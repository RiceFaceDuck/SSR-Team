    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex justify-center items-center p-4">
      <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200 border border-white/50">
        <div className="flex justify-between items-center p-6 border-b border-slate-200/50 bg-gradient-to-r from-slate-50 to-white">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition-colors bg-white/50 p-1.5 rounded-lg border border-slate-200 hover:bg-slate-100 active:scale-95">&times;</button>
        </div>
        
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto flex-1 space-y-4">
          {error && <div className="bg-red-50/80 backdrop-blur-sm border border-red-200 text-red-700 px-4 py-3 rounded-xl shadow-sm text-sm font-medium animate-in fade-in">{error}</div>}
          
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">ID (Unique)</label>
              <input 
                type="text" 
                name="id" 
                value={formData.id} 
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">ชื่อผู้จัดการทีม</label>
              <input 
                type="text" 
                name="name" 
                value={formData.name} 
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">ลิงก์รูปภาพ (Avatar URL)</label>
              <input 
                type="text" 
                name="avatarUrl" 
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">คำอธิบายความสามารถ</label>
            <textarea 
              name="description" 
          ...
          <div className="flex items-center pt-2 gap-2">
            ...
            />
            <label htmlFor="isActiveMgr" className="text-sm font-bold text-slate-700 cursor-pointer select-none">
