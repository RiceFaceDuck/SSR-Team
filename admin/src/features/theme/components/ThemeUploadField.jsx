...
export default function ThemeUploadField({ 
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
      ...
      <div className={`w-full h-48 rounded-xl mb-4 overflow-hidden border-2 border-dashed relative group flex items-center justify-center ${isObjectPreview ? 'bg-slate-900 border-slate-700' : 'bg-slate-100 border-slate-300'}`}>
        {(selectedFile || currentUrl) ? (
          <img 
            src={selectedFile ? URL.createObjectURL(selectedFile) : currentUrl} 
            alt="Preview" 
            className={isObjectPreview ? "w-32 h-32 object-contain animate-[bounce_3s_ease-in-out_infinite]" : "w-full h-full object-cover"}
          />
        ) : (
          <span className={`${isObjectPreview ? 'text-slate-500' : 'text-slate-400'} text-sm font-bold`}>ยังไม่มีรูปภาพ</span>
        )}
      </div>
