...
export default function HistoricalApiConfig({ onFetchStart }) {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      setIsFetching(false);
      alert(
        `ดึงข้อมูลเสร็จสิ้น: ${totalSaved} รายการ (เนื่องจากข้อจำกัด API อาจดึงข้อมูลมาบางส่วน)`
      );
    } catch (err) {
      console.error(err);
      ...
      <div className="bg-blue-50 border border-blue-100 p-4 rounded-2xl flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-blue-700 text-sm mt-1">
            การดึงข้อมูลทั้งฤดูกาลอาจใช้เวลาและกินโควต้า API-Sports
            กรุณาดึงเฉพาะข้อมูลที่จำเป็นเพื่อนำไปอ้างอิงใน Value Engine
          </p>
        </div>
      </div>
