    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden relative">
      {isLoading && teams.length > 0 && (
        <div className="absolute inset-0 bg-white/50 backdrop-blur-[1px] z-10 flex items-center justify-center">
          <span className="bg-white px-4 py-2 rounded-full shadow-md text-sm font-bold text-indigo-600 animate-pulse">กำลังอัปเดตข้อมูล...</span>
        </div>
      )}
      {isLoading && teams.length === 0 ? (
                ...
                <tr key={team.id} className="hover:bg-gray-50 transition-colors group">
                  ...
                  <td className="p-4">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => onEdit(team)} className="p-2 text-blue-600 hover:bg-blue-100 rounded-lg">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => onDelete(team.id, team.name)} className="p-2 text-red-600 hover:bg-red-100 rounded-lg">
