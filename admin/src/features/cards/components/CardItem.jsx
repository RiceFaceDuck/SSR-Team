...
export default function CardItem({ card, onEdit, onDelete }) {
  ...
  const getRarityBadge = () => {
    switch (rarity) {
      case 'LEGENDARY': return 'bg-amber-500 text-white shadow-sm shadow-amber-500/30';
      case 'EPIC': return 'bg-purple-500 text-white shadow-sm shadow-purple-500/30';
      case 'RARE': return 'bg-blue-500 text-white shadow-sm shadow-blue-500/30';
      case 'COMMON': 
  ...
  return (
    <div className={`border rounded-2xl p-5 transition-all duration-300 group flex flex-col h-full relative overflow-hidden ${getRarityStyles()}`}>
      {/* Rarity Glow Effect */}
      {rarity === 'LEGENDARY' && (
      ...
      <div className="flex justify-between items-start mb-3 relative z-10">
        ...
        <div className="flex flex-col items-end gap-1">
          <span className={`px-2 py-0.5 text-[9px] font-black tracking-wider rounded-md uppercase ${getRarityBadge()}`}>
            {rarity}
          </span>
          <span className={`px-2 py-0.5 text-[10px] font-bold rounded-md border ${card.isActive ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>
      ...
      <p className="text-sm text-slate-500 mb-4 flex-1 relative z-10 leading-relaxed">{card.description}</p>
