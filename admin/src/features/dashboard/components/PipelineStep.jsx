...
export default function PipelineStep({ 
  ...
  return (
    <div className={`relative flex flex-col md:flex-row items-center md:items-start gap-4 p-4 rounded-2xl transition-all ${isActive ? styles.active : 'opacity-60 grayscale'}`}>
      <div className={`w-16 h-16 rounded-full flex items-center justify-center shrink-0 shadow-lg ${isActive ? styles.icon : 'bg-slate-200 text-slate-500'}`}>
        <Icon size={28} />
      </div>
      <div className="text-center md:text-left flex-1">
        <h3 className="font-black text-lg text-slate-800">{title}</h3>
        <p className="text-sm font-medium text-slate-600 mt-1" dangerouslySetInnerHTML={{ __html: description }}></p>
      </div>
      {isActive && children && (
