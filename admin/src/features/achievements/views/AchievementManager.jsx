    ...
    <div className="space-y-6">
      ...
      {isFormOpen && (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <h3 className="font-bold text-lg mb-4 border-b pb-2">{editingItem ? 'แก้ไขฉายา' : 'สร้างฉายาใหม่'}</h3>
          <AchievementForm 
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td colSpan="5" className="p-8 text-center text-slate-400">ยังไม่มีข้อมูลฉายา</td></tr>
              ) : (
                  ...
                  <tr key={achv.id} className="hover:bg-slate-50">
                    ...
                    <td className="p-4">
                      <span className={`px-2 py-1 rounded text-xs font-bold ${
                        achv.rarity === 'legendary' ? 'bg-amber-100 text-amber-700' :
                        achv.rarity === 'epic' ? 'bg-purple-100 text-purple-700' :
                        achv.rarity === 'rare' ? 'bg-blue-100 text-blue-700' :
                    ...
                    <td className="p-4 text-sm font-medium text-slate-700">
                      {achv.conditionType === 'none' ? 'ให้ทุกคน' : 
                       achv.conditionType === 'admin' ? 'เฉพาะแอดมิน' : 
                       `${achv.conditionType} >= ${achv.conditionValue}`}
                    </td>
                    <td className="p-4">
                      <span className={`w-3 h-3 rounded-full inline-block ${achv.isActive ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                    </td>
                    <td className="p-4 text-right">
                      <button onClick={() => { setEditingItem(achv); setIsFormOpen(true); }} className="p-2 text-slate-400 hover:text-blue-600 rounded-lg">
                        <Edit2 size={16} />
                      </button>
                      <button onClick={() => handleDelete(achv.id)} className="p-2 text-slate-400 hover:text-red-600 rounded-lg">
                        <Trash2 size={16} />
                      </button>
                    </td>
                  </tr>
