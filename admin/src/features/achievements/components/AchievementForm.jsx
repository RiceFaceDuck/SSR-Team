    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select name="iconType" value={formData.iconType} onChange={handleChange} className="w-full border rounded-lg p-2">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div>
          ...
          <select name="rarity" value={formData.rarity} onChange={handleChange} className="w-full border rounded-lg p-2">
        ...
        <div>
          <label className="block text-sm font-bold text-slate-700 mb-1">เงื่อนไขการตรวจสอบ (Condition)</label>
          <select name="conditionType" value={formData.conditionType} onChange={handleChange} className="w-full border rounded-lg p-2">
        ...
        <div>
          <label className="block text-sm font-bold text-slate-700 mb-1">ค่าเป้าหมาย (Target Value)</label>
          <input 
            type="number" 
            name="conditionValue" 
      ...
      <div className="flex items-center gap-2 mt-4">
        <input type="checkbox" name="isActive" id="isActive" checked={formData.isActive} onChange={handleChange} className="w-4 h-4 rounded text-blue-600" />
        <label htmlFor="isActive" className="text-sm font-bold text-slate-700">เปิดใช้งาน (Active)</label>
      </div>

      <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
        <button type="button" onClick={onCancel} className="px-4 py-2 border rounded-lg text-slate-600 hover:bg-slate-50 font-bold flex items-center gap-2">
          <X size={16} /> ยกเลิก
        </button>
        <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-bold flex items-center gap-2">
          <Save size={16} /> บันทึก
        </button>
      </div>
    </form>
