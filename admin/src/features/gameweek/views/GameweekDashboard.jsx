...
export default function GameweekDashboard() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-6 flex justify-between items-start">
        <div>
          ...
          <p className="text-slate-500 mt-2">
            ควบคุมสัปดาห์การแข่งขัน, กรอกสถิติ, และคำนวณคะแนนรวม
          </p>
        </div>
        <button
      ...
      {showGuide && (
        <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-5 mb-8 text-sm text-indigo-900 shadow-sm">
          ...
          <ol className="list-decimal list-inside space-y-2 opacity-90">
            <li>
              <strong>แท็บ "ตั้งค่าสัปดาห์":</strong> ใช้สำหรับเปิด/ปิดตลาดซื้อขาย หรือจัดการเวลา
              Deadline ของแต่ละสัปดาห์
            </li>
            <li>
              <strong>แท็บ "อัปเดตสถิตินักเตะ":</strong> เมื่อการแข่งขันจบลง ให้แอดมินนำสถิติจริง
              (ยิง, จ่าย, คลีนชีต) มากรอกใส่รายชื่อนักเตะ และกด "บันทึก" ทีละคน
            </li>
            <li>
              <strong>แท็บ "ประมวลผลคะแนน":</strong> พิมพ์ชื่อรอบ (เช่น GW1) แล้วกดคำนวณ
              ระบบจะดึงสถิติล่าสุดไปแจกคะแนนให้ผู้เล่นทุกคน และจัดอันดับ Leaderboard อัตโนมัติ
            </li>
          </ol>
          <div className="mt-4 p-3 bg-indigo-100 rounded-lg text-xs font-medium border border-indigo-200">
            <span className="text-red-600 font-bold">ข้อควรระวัง:</span> การกด "ประมวลผลคะแนน"
            ควรทำเมื่อแข่งขันจบครบทุกคู่ในสัปดาห์นั้นแล้ว และห้ามกดซ้ำสำหรับ GW เดิม
            เพราะคะแนนอาจจะถูกบวกเบิ้ล
          </div>
        </div>
      )}
      ...
      {activeTab === 'live' && <LiveMatchController />}

      {activeTab === 'fixtures' && <GameweekFixtures gameweekId={gwId} />}

      {activeTab === 'stats' && <PlayerStatsEntry />}

      {activeTab === 'process' && <GameweekProcessPanel initialGwId={gwId} />}
    </div>
  );
}
