...
export default function ApiSettingsPanel() {
  ...
  const handleSave = () => {
    apiFootballService.setConfig(config.leagueId, config.season);
  ...
  return (
    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        {/* API Key field removed because it is now managed via Cloud Functions */}
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">
            League ID (ค่าเริ่มต้น 39 = พรีเมียร์ลีก)
          </label>
          <input
            type="text"
          ...
          <div className="w-full p-4 bg-indigo-50 rounded-lg border border-indigo-100 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-indigo-700">
                ดึงข้อมูลอัตโนมัติทุกๆ 15 นาที (เมื่อเปิดหน้านี้ทิ้งไว้)
              </p>
            </div>
