  ...
  return (
    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
      <label className="block text-sm font-bold text-slate-800 mb-1 flex items-center gap-2">
        {themeKey === 'floatingObjectUrl' ? <Sparkles size={16} className="text-amber-500"/> : <ImageIcon size={16} className="text-blue-500"/>}
        {label}
      </label>
      ...
      <div className="flex gap-2 items-center">
        ...
        <label className={`flex-shrink-0 cursor-pointer flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${isUploading ? 'bg-slate-200 text-slate-400' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-sm'}`}>
      ...
      {historyList.length > 0 && (
        <div className="mt-3 pt-3 border-t border-slate-100">
          <p className="text-xs text-slate-500 mb-2 font-medium">🕒 ประวัติรูปภาพที่เคยอัปโหลด (คลิกเพื่อใช้งานทันที):</p>
