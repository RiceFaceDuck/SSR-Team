  ...
  const logicTypes = [
    {
      ...
      description: 'กัปตันคูณสาม: นำคะแนนของผู้เล่นที่สวมการ์ดใบนี้คูณ 3 อัตโนมัติ (ไม่ต้องกรอก Value)',
    ...
    },
    {
      id: 'BENCH_BOOST',
      title: 'BENCH_BOOST',
      description: 'สำรองรับทรัพย์: นำคะแนนของผู้เล่นตัวสำรองทุกคนมาคิดรวมในสัปดาห์นั้น (ใช้ในระดับ Manager)',
    ...
    },
    {
      ...
      description: 'โบนัสตัวจริง: หากผู้เล่นคนนี้ไม่ได้ถูกเปลี่ยนตัวออกในโลกจริง จะได้คะแนนพิเศษตาม Value (เช่น +2)',
    ...
    },
    {
      ...
      description: 'ปลดล็อกแผนการเล่น: (มักใช้กับ Manager) เพิ่มแผนการเล่นใหม่เข้าสู่ระบบ Value: Array ของแผน',
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 flex flex-col min-h-[calc(100vh-8rem)] p-8 overflow-y-auto">
      <div className="mb-8 border-b border-slate-100 pb-6 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-500 mt-1">คู่มือสำหรับแอดมินในการกำหนด Effect Logic เพื่อสร้างความสามารถพิเศษให้ การ์ด หรือ ผู้จัดการทีม</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {logicTypes.map((logic) => (
          <div key={logic.id} className="bg-slate-50 rounded-2xl p-6 border border-slate-200 hover:shadow-lg hover:border-slate-300 transition-all group">
            <div className="flex items-start gap-4 mb-4">
              <div className={`${logic.bgColor} p-3 rounded-xl shadow-sm group-hover:scale-110 transition-transform`}>
                {logic.icon}
              </div>
            ...
            <div className="bg-slate-800 rounded-xl p-3 relative overflow-hidden">
              <span className="absolute top-0 right-0 bg-slate-700 text-slate-300 text-[9px] font-bold px-2 py-0.5 rounded-bl-lg">JSON</span>
      ...
      <div className="mt-12 bg-blue-50 border border-blue-200 rounded-2xl p-6">
        ...
        <p className="text-slate-600 text-sm leading-relaxed">
          หากในอนาคตคุณมีไอเดียใหม่ๆ คุณสามารถเลือก <strong>"พิมพ์กำหนดเอง (Custom)"</strong> ในหน้าสร้างการ์ด และใส่โค้ด Logic Type เป็นภาษาอังกฤษพิมพ์ใหญ่ (เช่น <code>FREE_TRANSFER</code>) จากนั้นระบบ Frontend (คนเขียนโค้ด) จะสามารถนำคีย์เวิร์ดนี้ไปเขียนดักเงื่อนไขในการคิดคะแนนหรือระบบในเกมต่อไปได้ทันทีครับ
        </p>
      </div>
    </div>
  );
