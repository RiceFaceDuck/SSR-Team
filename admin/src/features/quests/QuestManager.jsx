...
import { Plus, Edit2, Trash2, ExternalLink, Activity, CheckCircle, Clock, Trophy } from 'lucide-react';
import { useQuestStore } from '../../store/questStore';
import QuestFormModal from './QuestFormModal';

export default function QuestManager() {
  // ดึง State และ Functions จาก Store
  const { quests, isLoading, fetchQuests, addQuest, updateQuest, deleteQuest, toggleStatus } = useQuestStore();
  
  // Local State สำหรับควบคุม Modal
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingQuest, setEditingQuest] = useState(null);

  // โหลดข้อมูลครั้งแรกเมื่อเปิดหน้านี้
  useEffect(() => {
    fetchQuests();
  }, [fetchQuests]);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-800 tracking-tight flex items-center gap-2">
            ...
            จัดการสปอนเซอร์ & ภารกิจ
          </h1>
          <p className="text-slate-500 mt-1">ระบบเพิ่มโฆษณาและตั้งค่าการแจกรางวัล (Balls ⚽) ให้ผู้เล่น</p>
        </div>
        <button 
