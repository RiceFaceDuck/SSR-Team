...
export const gameweekCalculationService = {
  ...
  processGameweek: async (gameweekId) => {
    try {
      ...
      const captainMultiplier = gameRules.captainMultiplier?.isActive
        ? gameRules.captainMultiplier?.value || 2
        : 1;
        ...
        for (const { userDoc, squadSnap } of chunkResults) {
          ...
          if (mySquad && Array.isArray(mySquad)) {
            for (const playerItem of mySquad) {
              ...
              const isPlaying =
                playerItem.isStarting ||
                playerItem.appliedCard?.effectLogic?.type === 'BENCH_BOOST';
              ...
              if (isPlaying) {
                ...
                if (pData) {
                  ...
                  hasPlayed =
                    pData.stats?.minutes > 0 || pData.stats?.played > 0 || pointsEarned > 0;
                }
              }
      ...
      await writeBatch(db)
        .set(gwStatusRef, { status: 'completed', updatedAt: serverTimestamp() }, { merge: true })
        .commit();
